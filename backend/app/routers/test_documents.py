...
async def create_document(payload: TestDocumentCreate, db: AsyncSession = Depends(get_db)):
...
async def update_document(
    doc_id: str, payload: TestDocumentUpdate, db: AsyncSession = Depends(get_db)
):
    doc = await db.get(TestDocument, doc_id)
    if not doc:
        raise HTTPException(404, "Document not found")
