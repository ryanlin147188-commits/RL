...
ensure_mysql_admin() {
    step "確保 MySQL admin 使用者存在"
    # 嘗試新的 admin/admin123，若失敗再試舊的 root/password 並建立 admin
    local ok=0
    for pwd in admin123 password; do
        if docker exec autotest-mysql mysql -uroot -p"$pwd" -e "SELECT 1;" >/dev/null 2>&1; then
            docker exec autotest-mysql mysql -uroot -p"$pwd" -e \
    ...
    if (( ok == 1 )); then
        success "admin 使用者已就緒"
    else
        warn "無法連進 MySQL；若這是第一次啟動可忽略（init 時會自動建立）"
    fi
}
...
${GREEN}${BOLD}════════════════════════════════════════════════════════════${NC}
  ...
  🗄  MinIO：${CYAN}http://localhost:9001/${NC}      (admin / admin123)
$(echo -e "$apple_note")
  常用指令：
...
case "$CMD" in
    ...
    --reset|reset)
        require_docker; check_project_dir
        warn "此動作會刪除所有 MySQL / MinIO volumes，測試資料、報告、截圖都會消失！"
