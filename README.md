...
> 讓 QA、開發與產品團隊在同一份資產上協作。**Apache 2.0 全開源,自架到內網即可,內建 AI 助理。**
...
| 項目 | 商業 SaaS(TestRail + Tricentis 為例)| RL |
|---|---|---|
...
| 🤖 **內建 AI 助理** ✨ | 右下角浮動圓點 → 對話框,支援 OpenAI / Anthropic / DeepSeek / Groq / Ollama 等 11 家 + 自架;**用 token 即時拉模型清單**,o1 / DeepSeek-R1 等推理模型自動偵測支援思考程度 |
| 🎯 **多方法論支援** | ATDD / BDD / KDT / DDT / TDD / SBE / FDD 都能在平台內自然表達 |
| 🏗 **完整 ALM** | 測試計畫(ISTQB 8 區塊)/ 需求 / 缺陷 / 里程碑 / WBS / 文件 / Backlog / 排程 / 通知 |
| 🔐 **企業級 Auth** | JWT 雙 token、bcrypt 密碼、Fernet 加密 secret、**大頭貼上傳**、**角色設定**、**多 AI provider** |
...
從 [GitHub Releases](https://github.com/ryanlin147188-commits/RL_TMP/releases) 下載 `autotest-images-1.0.0.tar`(離線散佈包,含 backend / celery / runner / frontend 4 個 image),在你的 VM 上:
...
| **AI 助理** ✨ | 右下角浮動 FAB → 滑入式對話框,Ctrl+/ 熱鍵;支援 OpenAI / Anthropic / DeepSeek / Groq / OpenRouter / Mistral / xAI / Google / Ollama / LM Studio + 自架;**「用 token 拉模型清單」** 一鍵載入該 provider 提供的所有模型;o1 / o3 / DeepSeek-R1 等**自動標記支援思考程度**(low/medium/high) |
| **使用者帳戶** ✨ | **大頭貼上傳**(SeaweedFS,5 MB 內)、可改顯示名稱 / Email / 角色;JWT 雙 token、bcrypt 密碼、Fernet 加密 secret;一鍵切換亮/暗主題、繁中/英文 |
...
<p align="center">
<b>RL v1.0</b> — 讓自動化測試回歸簡單、透明、可追溯。<br>
