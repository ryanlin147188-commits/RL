...
- **API 閘道器**：**Apache APISIX**（port 9080）為對外 API 入口，含限流 / CORS / request-id 等 plugin
- **集中式 log 觀測**：**Fluent Bit** 採 host docker.json-file logs → 推 **VictoriaLogs**（port 9428，自帶 vmui 查詢面板）
- **零 Copyleft 風險**：整套 stack 全 Apache 2.0 / BSD-3 / PostgreSQL License — 商業 SaaS 部署無授權義務
